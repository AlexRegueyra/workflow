...
const SpreadsheetNodeConfig = ({ node, onChange }) => {
    ...
    // En el método renderExcelFileFields() de SpreadsheetNodeConfig
    const renderExcelFileFields = () => (
        <div>
            <div className="mb-3">
                <label className="block text-sm font-medium mb-1">Subir archivo Excel</label>
                <input
                    type="file"
                    accept=".xls,.xlsx"
                    className="w-full p-2 border rounded"
                ...
                />
                {config.isUploading && (
                    <p className="text-xs text-blue-500 mt-1">Subiendo archivo...</p>
                )}
                {config.filePath && (
                    <div className="text-xs text-green-500 mt-1">
                        <p>Archivo subido: {config.fileName}</p>
                        <p className="text-gray-500">Tamaño: {(config.fileSize / 1024).toFixed(2)} KB</p>
                    </div>
                )}
            </div>
    ...
    const renderCSVFields = () => (
        <div>
            <div className="mb-3">
                <label className="block text-sm font-medium mb-1">Subir archivo CSV</label>
                <input
                    type="file"
                    accept=".csv"
                    className="w-full p-2 border rounded"
    ...
    //  botón al final del JSX retornado
    <button
